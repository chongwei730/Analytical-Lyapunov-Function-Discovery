...
conf.exp.seed_runs = int(os.environ.get("LYA_SEED_RUNS", 1))
conf.exp.n_cores_task = 1
conf.exp.seed_start = int(os.environ.get("LYA_SEED_START", 5))
conf.exp.baselines = ["transformer"]
# User must specify the benchmark to run (overridable via env for batch runs):
conf.exp.benchmark = os.environ.get("LYA_BENCHMARK", "fn_d_all_o")
if os.environ.get("LYA_NSAMPLES"):
    conf.exp.n_samples = int(os.environ["LYA_NSAMPLES"])
