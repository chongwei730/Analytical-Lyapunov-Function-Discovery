...
def dynamics():
    x1, x2, x3, x4, x5, x6, x7, x8, x9 = sym.symbols("x1, x2, x3, x4, x5, x6, x7, x8, x9")

    ## G.6 9-D Synthetic Dynamics (adapted from Appendices F.4 & G.2)
    ## fn_d_all_l_5 is the corresponding state space and training set setting (|xi| <= 1.5)
    ## paper V = sum_{i=1}^6 xi^2 + sin(x7)^2 + x8^2 - cos(x9) + 1

    state_variables = [x1, x2, x3, x4, x5, x6, x7, x8, x9]
    dynamics_ode = [-x1 + 0.5 * x2 - 0.1 * x5 ** 2,
                    ...
                    -0.5 * x5 - x6 + 0.1 * x2 ** 2,
                    x8,
                    -sym.sin(x7) * sym.cos(x7) - x8 - sym.sin(x9) * sym.cos(x9) - 0.1 * x2,
                    x8 - x9]
