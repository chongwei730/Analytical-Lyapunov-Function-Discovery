...
def config_factory():
    return {
        ...
        "prior": {
            ...
            "soft_length": {"loc": 14, "scale": 5, "on": True},
        },
    }
...
def dynamics():
    ...
    ## F.3 Three-variable polynomial system with higher degree (3-D), system 2 of Table 6
    ## fn_d_all_d_5 is the corresponding state space and training set setting
    ## Domain D = {|xi| <= 1}; paper V = x1^8 * x2^2 * x3^2 + x2^2

    state_variables = [x1, x2, x3]
    dynamics_ode = [-8 * x1 * x2 ** 2 - 10 * x2 ** 4,
                    -8 * x2 ** 3 + 3 * x2 ** 2 - 8 * x2,
                    -x3]
