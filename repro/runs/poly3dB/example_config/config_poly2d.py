...
def dynamics():
    ...
    ## F.2 Two-variable polynomial system with higher degree (2-D)
    ## fn_d_all_b_5 is the corresponding state space and training set setting
    ## Domain D = {|xi| <= 1}; paper V = 9*x1^2 + x2^2

    state_variables = [x1, x2]
    dynamics_ode = [-5 * x1 ** 3 - 2 * x1 * x2 ** 2,
                    -9 * x1 ** 4 + 3 * x1 ** 3 * x2 - 4 * x2 ** 3]
