...
def dynamics():
    ...
    ## G.3 N-bus Lossless Power System, 3-bus case (6-D)
    ## fn_d_all_e_5 is the corresponding state space and training set setting
    ## States: x1,x2,x3 = delta1,delta2,delta3 (|.|<=0.75); x4,x5,x6 = omega1,omega2,omega3 (|.|<=1.2)
    ## Params: p_i=0, m_i=2, d_i=1, u_i(w)=w, B_ij=1 (i!=j), B_ii=0
    ## m_i*wdot_i = -d_i*w_i - u_i - sum_{j!=i} B_ij*sin(delta_i-delta_j)
    ## paper V = sum w_i^2 - 0.5*(sum_{i!=j} cos(delta_i-delta_j) - 1)

    state_variables = [x1, x2, x3, x4, x5, x6]
    dynamics_ode = [x4,
                    x5,
                    x6,
                    -x4 - 0.5 * (sym.sin(x1 - x2) + sym.sin(x1 - x3)),
                    -x5 - 0.5 * (sym.sin(x2 - x1) + sym.sin(x2 - x3)),
                    -x6 - 0.5 * (sym.sin(x3 - x1) + sym.sin(x3 - x2))]
