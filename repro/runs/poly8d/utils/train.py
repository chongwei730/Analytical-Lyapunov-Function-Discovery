...
# PGD verification device. Authors hardcoded "cuda:1" (a 2nd GPU); make it
# configurable so single-GPU allocations (cuda:0) work. Override with LYA_PGD_DEVICE.
PGD_DEVICE = os.environ.get("LYA_PGD_DEVICE", "cuda:0")
# Cap multiprocessing pools to the CPUs actually allocated by SLURM. The repo
# used mp.cpu_count() (the whole node), spawning dozens of workers onto a few
# cgroup-confined cores -> severe oversubscription and ~20-60x slowdown.
N_POOL = int(os.environ.get("LYA_POOL_PROCS") or os.environ.get("SLURM_CPUS_PER_TASK") or mp.cpu_count())
...
def optomize_at_test(
    ...
    for epoch in range(n_epochs):
        ...
        with mp.Pool(processes= N_POOL, initializer=init_worker) as pool:
        ...
        if any(success) == 0 and (run_gp_meld):
            ...
            # Fix published-repo typo: this array is read below as `gp_collection_reward`.
...
def pgd_check(sympy, X, low_bound, up_bound):
    ...
    sympy_torch = sympytorch.SymPyModule(expressions=[sympy]).to(PGD_DEVICE)
    data = torch.rand((X.shape[0], X.shape[1])).to(PGD_DEVICE)
    result = pgd_attack(data, sympy_torch, min(up_bound)*0.8, steps=100, lower_boundary=torch.tensor(low_bound).to(PGD_DEVICE), upper_boundary=torch.tensor(up_bound).to(PGD_DEVICE), direction="minimize")
