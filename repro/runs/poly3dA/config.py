...
def dynamics():
    ...
    ## F.3 Three-variable polynomial system with higher degree (3-D), system 1 of Table 6
    ## fn_d_all_c_5 is the corresponding state space and training set setting
    ## Domain D = {|xi| <= 1}; paper V = 9*x1^2 + x2^2 + x3^2

    state_variables = [x1, x2, x3]
    dynamics_ode = [-3 * x1 ** 3 + 3 * x1 * x3 - 9 * x1,
                    -x1 ** 3 - 5 * x2 + 5 * x3 ** 2,
                    -9 * x3 ** 3]
